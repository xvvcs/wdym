...
@Test func clarifyPromptPreservesSemanticParentheticalContent() {
    let service = PromptRefactorService()
    let output = "Write a script using Python (3.12) to connect via HTTPS (TLS 1.3) and fetch user records."

    let clarified = service.clarifyPrompt(output)

    #expect(clarified == "Write a script using Python (3.12) to connect via HTTPS (TLS 1.3) and fetch user records.")
}
