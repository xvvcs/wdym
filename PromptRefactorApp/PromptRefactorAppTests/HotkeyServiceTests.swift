...
struct HotkeyServiceTests {
  ...
  @Test func globalHotkeyServicesUseDistinctEventIDs() {
    let first = GlobalHotkeyService()
    let second = GlobalHotkeyService()

    #expect(first.eventHotKeyID.id != second.eventHotKeyID.id)
    #expect(first.matches(eventHotKeyID: first.eventHotKeyID))
    #expect(!first.matches(eventHotKeyID: second.eventHotKeyID))
    #expect(first.handlingStatus(for: second.eventHotKeyID) == OSStatus(eventNotHandledErr))
  }
